...
const HistoryView: React.FC<HistoryViewProps> = ({ tasks, projects, toggleTask, deleteTask, updateProject, deleteProject }) => {
  const [deleteData, setDeleteData] = useState<{ type: 'task' | 'project', id: string } | null>(null);

  const handleDeleteClick = (type: 'task' | 'project', id: string) => {
    ...
    <div className="fade-in space-y-12 py-2">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-br from-[#1c2621] to-transparent p-6 rounded-[2rem] border border-white/5">
        ...
        <div className="flex flex-col items-center bg-[#0d1310] p-4 rounded-2xl border border-white/10 shadow-xl min-w-[150px]">
          ...
          <div className="text-3xl font-black text-[#4ade80] tracking-tighter shadow-emerald-500/20">{completedTasks.length + completedProjects.length}</div>
        </div>
      </div>
      ...
      <div className="relative group max-w-2xl mx-auto px-4">
        ...
        <input
          type="text"
          placeholder="성취 기록을 키워드로 검색 (프로젝트 및 과업)..."
          ...
          <div className="bg-[#1c2621] w-full max-w-sm rounded-[2rem] border border-red-500/20 shadow-2xl overflow-hidden">
            <div className="p-8 text-center space-y-4">
              ...
              <p className="text-gray-400 text-xs leading-relaxed">
                이 작업은 되돌릴 수 없습니다.<br />
                정말로 이 {deleteData.type === 'project' ? '프로젝트' : '과업'} 기록을 삭제하시겠습니까?
              </p>
            </div>
